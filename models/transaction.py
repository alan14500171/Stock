...
class StockTransaction:
    ...
    def get_by_code(transaction_code):
        """根据交易编号获取交易记录"""
        sql = "SELECT * FROM stock_transactions WHERE transaction_code=%s"
    ...
    def get_user_transactions(user_id, market=None, stock_code=None, 
        ...
        """获取用户的交易记录"""
        sql = "SELECT * FROM stock_transactions WHERE user_id=%s"
        params = [user_id]
...
class TransactionDetail:
    ...
    def get_by_transaction_id(transaction_id):
        """获取交易的所有明细"""
        sql = "SELECT * FROM stock_transaction_details WHERE transaction_id=%s ORDER BY id"
