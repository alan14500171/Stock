...
"""
重新计算transaction_splits表中的字段
使用方法：
    python recalculate_transaction_splits.py  # 处理所有记录
    python recalculate_transaction_splits.py --holder_id=1  # 只处理指定持有人的记录
    python recalculate_transaction_splits.py --stock_code=00700 --market=HK  # 只处理指定股票的记录
    python recalculate_transaction_splits.py --date=2023-01-01  # 只处理指定日期及之后的记录
    python recalculate_transaction_splits.py --transaction_id=123  # 只处理指定交易及其后续交易
    python recalculate_transaction_splits.py --update_original  # 同时更新原始交易记录
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(f'logs/recalculate_transaction_splits_{datetime.now().strftime("%Y%m%d")}.log')
    ]
)
...
def main():
    ...
    try:
        # 解析命令行参数
        parser = argparse.ArgumentParser(description='重新计算交易分单记录字段')
        ...
        parser.add_argument('--transaction_id', type=int, help='指定交易ID，只处理该交易及其后续交易')
        parser.add_argument('--update_original', action='store_true', help='是否同时更新原始交易记录')
        args = parser.parse_args()
        
        ensure_log_directory()
        logger.info("开始重新计算交易分单记录字段")
        ...
        logger.info(f"重新计算交易分单记录字段完成: 总计 {total} 条, 成功 {success} 条, 失败 {fail} 条")
